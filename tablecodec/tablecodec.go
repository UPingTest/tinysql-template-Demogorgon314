...
func DecodeRecordKey(key kv.Key) (tableID int64, handle int64, err error) {
	/* Your code here */
	if key == nil {
		return 0, 0, errors.New("key is nil")
	}
	if !hasTablePrefix(key) {
		return 0, 0, errors.New("key verification failed")
	}
...
func DecodeIndexKeyPrefix(key kv.Key) (tableID int64, indexID int64, indexValues []byte, err error) {
	/* Your code here */
	if key == nil {
		return 0, 0, nil, errors.New("key is nil")
	}
	if !hasTablePrefix(key) {
		return 0, 0, nil, errors.New("key verification failed")
	}
	key, tableID, err = codec.DecodeInt(key[len(tablePrefix):])
	if err != nil {
...
func hasIndexPrefixSep(key kv.Key) bool {
	return key[0] == recordPrefixSep[0] && key[1] == indexPrefixSep[1]
}
